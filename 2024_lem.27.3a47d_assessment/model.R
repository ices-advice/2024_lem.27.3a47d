### ------------------------------------------------------------------------ ###
### run surbar and advice rule ####
### ------------------------------------------------------------------------ ###

## Before: bootstrap/data/ns_lem_sw.dat
##         bootstrap/data/ns_lem_mo.dat
##         bootstrap/data/ns_lem_gam.dat
## After:  model/surbar_output_noage1q1.pdf
...
if (pdf.plots)
{
  pdf(file = "model\\surbar_output_noage1q1.pdf",
...
# Define standard run settings
x.surv <-"q1q3" #"q1q3"		# Surveys to use
x.ref <- 3			# Reference age
x.lambda <- 3		# Smoothing parameter
x.q <- 1.0			# Baseline q catchability
  ...
  for (i.ref in s.ref)
  {
    for (i.lambda in s.lambda)
    {
      for (i.q in s.q)
      {
        ...
        # North Sea lemon sole WGNSSK 2024
        sw.file <- paste0(data.path,"ns_lem_sw.dat")
        mat.file <- paste0(data.path,"ns_lem_mo.dat")
        survey.file <- paste0(data.path,"ns_lem_gam.dat")
        zbarage.1 <- 3   # TM corrected from 2,4
        zbarage.2 <- 5 
        ref.age <- i.ref 
        lambda <- i.lambda
        spawn.time <- 0.5
        ...
        # 1. Reduce catchability on youngest ages (remove hooks in catch curves)
        ...
        fts.mean.rec <- mean(stock$rec)
        fts.mean.ssb <- mean(stock$ssb)
        fts.mean.tsb <- mean(stock$tsb)
        stock$rec <- stock$rec / fts.mean.rec
        stock$ssb <- stock$ssb / fts.mean.ssb
        stock$tsb <- stock$tsb / fts.mean.tsb
...
if(FALSE){
  ...
  list.condition <- sapply(s.list, function(x){x$ref == x.ref & x$lambda == x.lambda & x$q == x.q})
...
if(FALSE){
  ...
  list.condition <- sapply(s.list, function(x){x$surv == x.surv & x$lambda == x.lambda & x$q == x.q})
  s0.list  <- s.list[list.condition]
  if (!pdf.plots) windows(width = 10, height = 7)
  par(mfrow = c(2,2), mar = c(3,4,2,1))
  # Mean Z
...
if(FALSE){
  ...
  # Effect of lambda
  list.condition <- sapply(s.list, function(x){x$surv == x.surv & x$ref == x.ref & x$q == x.q})
  s0.list  <- s.list[list.condition]
  if (!pdf.plots) windows(width = 10, height = 7)
  par(mfrow = c(2,2), mar = c(3,4,2,1))
  # Mean Z
...
if(FALSE){
  ...
  # Effect of q
  list.condition <- sapply(s.list, function(x){x$surv == x.surv & x$ref == x.ref & x$lambda == x.lambda})
  s0.list  <- s.list[list.condition]
  if (!pdf.plots) windows(width = 10, height = 7)
  par(mfrow = c(2,2), mar = c(3,4,2,1))
  # Mean Z
  ...
  stop() # Stop here when only doing sensitivity loop - then rerun using standard settings
...
for (i in 1:n.psim)
{
  ...
  x.psim.stock[[i]]$rec <- exp(x.r[na:length(r)]) / fts.mean.rec
  x.psim.stock[[i]]$ssb <- apply(n * sw * mat, 1, sum) / fts.mean.ssb
  x.psim.stock[[i]]$tsb <- apply(n * sw, 1, sum) / fts.mean.tsb
}
...
if(!pdf.plots) {
    savePlot(filename = "report/SURBAR_sum_line",type = "tiff")
dev.off()}

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "sum.boxplot", pdf.plots)
if(!pdf.plots) {savePlot(filename = "report/SURBAR_sum_box",type = "tiff")
dev.off()}

#save a plot per quarter
plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "res.line", pdf.plots, filename1= "report/SURBAR_res_line_Q1",filename2= "report/SURBAR_res_line_Q3" )


#save a plot per quarter
plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "res.smooth", pdf.plots, filename1="report/SURBAR_res_smooth_Q1",filename2="report/SURBAR_res_smooth_Q3")


plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "params", pdf.plots)
if(!pdf.plots){ savePlot(filename = "report/SURBAR_params",type = "tiff")
  dev.off()}


plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "catch.curve", pdf.plots)
if(!pdf.plots){ savePlot(filename = "report/SURBAR_catch_curves",type = "tiff")
  dev.off()}

plot.surbar(x = x, stock = stock, psim = x.psim.stock, psim.s = x.psim.s, psim.f = x.psim.f, psim.r = x.psim.r, 
            res = res, y1 = y1, y2 = y2, a1 = a1, a2 = a2, type = "log.by.cohort", pdf.plots)
if(!pdf.plots){ savePlot(filename = "report/SURBAR_log_by_cohort",type = "tiff")
  dev.off()}
...
if(!pdf.plots){ savePlot(filename = "report/SURBAR_corrQ1",type = "tiff")
  dev.off()}

if (!pdf.plots) windows(width = 10, height = 7)
plot.index.corr(wk.object = list(x[[2]][as.character(2006:2018),1:9]), wk.type = "SURBAR") # Q3
if(!pdf.plots){ savePlot(filename = "report/SURBAR_corr_Q3",type = "tiff")
  dev.off()}
...
for (ry in (y2-1):(y2-nretro))
{
  ...
  stockr$rec <- stockr$rec / fts.mean.rec
  stockr$ssb <- stockr$ssb / fts.mean.ssb
  stockr$tsb <- stockr$tsb / fts.mean.tsb
  
  retro.stock[[kk]] <- stockr
}

###################################################
# Calculate Mohn's rho for each summary statistic #
###################################################

# Form data frame for all retro mean Z estimates

y2 <- max(as.numeric(rownames(sw)))
ac <- function(wk){as.character(wk)}

retro.meanz <- data.frame(array(NA, dim = c(dim(sw)[1], nretro+1)))
rownames(retro.meanz) <- rownames(sw)
colnames(retro.meanz) <- y2:(y2-nretro)
retro.meanz[,1] <- stock$meanz
k <- 0
for (i in (y2-1):(y2-nretro))
{
  k <- k + 1
  nky <- length(retro.stock[[k]]$meanz)
  retro.meanz[1:nky,ac(i)] <- retro.stock[[k]]$meanz
}
# Calculate Mohn's rho for mean Z estimates
mohn.z <- 0
for (i in (y2-1):(y2-nretro+1))
{
  tf <- retro.meanz[ac(i),ac(y2)]
  tp <- retro.meanz[ac(i),ac(i)]
  mohn.z <- mohn.z + ((tp - tf)/tf)
  #print(round(c(tf,tp,(tp-tf)/tf,mohn.z),4))
}
mohn.z <- mohn.z / nretro

# Form data frame for all retro SSB estimates
retro.ssb <- data.frame(array(NA, dim = c(dim(sw)[1], nretro+1)))
rownames(retro.ssb) <- rownames(sw)
colnames(retro.ssb) <- y2:(y2-nretro)
retro.ssb[,1] <- stock$ssb
k <- 0
for (i in (y2-1):(y2-nretro))
{
  k <- k + 1
  nky <- length(retro.stock[[k]]$ssb)
  retro.ssb[1:nky,ac(i)] <- retro.stock[[k]]$ssb
}
# Calculate Mohn's rho for SSB estimates
mohn.ssb <- 0
for (i in (y2-1):(y2-nretro+1))
{
  tf <- retro.ssb[ac(i),ac(y2)]
  tp <- retro.ssb[ac(i),ac(i)]
  mohn.ssb <- mohn.ssb + ((tp - tf)/tf)
  #print(round(c(tf,tp,(tp-tf)/tf,mohn.ssb),4))
}
mohn.ssb <- mohn.ssb / nretro

# Form data frame for all retro TSB estimates
retro.tsb <- data.frame(array(NA, dim = c(dim(sw)[1], nretro+1)))
rownames(retro.tsb) <- rownames(sw)
colnames(retro.tsb) <- y2:(y2-nretro)
retro.tsb[,1] <- stock$tsb
k <- 0
for (i in (y2-1):(y2-nretro))
{
  k <- k + 1
  nky <- length(retro.stock[[k]]$tsb)
  retro.tsb[1:nky,ac(i)] <- retro.stock[[k]]$tsb
}
# Calculate Mohn's rho for TSB estimates
mohn.tsb <- 0
for (i in (y2-1):(y2-nretro+1))
{
  tf <- retro.tsb[ac(i),ac(y2)]
  tp <- retro.tsb[ac(i),ac(i)]
  mohn.tsb <- mohn.tsb + ((tp - tf)/tf)
  #print(round(c(tf,tp,(tp-tf)/tf,mohn.tsb),4))
}
mohn.tsb <- mohn.tsb / nretro

# Form data frame for all retro recruitment estimates
retro.rec <- data.frame(array(NA, dim = c(dim(sw)[1], nretro+1)))
rownames(retro.rec) <- rownames(sw)
colnames(retro.rec) <- y2:(y2-nretro)
retro.rec[,1] <- stock$rec
k <- 0
for (i in (y2-1):(y2-nretro))
{
  k <- k + 1
  nky <- length(retro.stock[[k]]$rec)
  retro.rec[1:nky,ac(i)] <- retro.stock[[k]]$rec
}
# Calculate Mohn's rho for recruitment estimates
mohn.rec <- 0
for (i in (y2-1):(y2-nretro+1))
{
  tf <- retro.rec[ac(i),ac(y2)] # Full
  tp <- retro.rec[ac(i),ac(i)]  # Peel
  mohn.rec <- mohn.rec + ((tf - tp)/tf)
  #print(round(c(tf,tp,(tp-tf)/tf,mohn.rec),4))
}
mohn.rec <- mohn.rec / nretro
...
legend(x = "topleft", legend = paste("Mohn's rho = ", round(mohn.z,2)),
       bty = "n", cex = 1.0)
...
legend(x = "topleft", legend = paste("Mohn's rho = ", round(mohn.ssb,2)),
       bty = "n", cex = 1.0)
...
legend(x = "topleft", legend = paste("Mohn's rho = ", round(mohn.tsb,2)),
       bty = "n", cex = 1.0)
...
legend(x = "topleft", legend = paste("Mohn's rho = ", round(mohn.rec,2)),
       bty = "n", cex = 1.0)
mtext("Retrospective analysis", outer = TRUE, side = 3, line = -1)

 savePlot(filename = "report/SURBAR_retro",type = "tiff")
  dev.off()

# save result table

results<-round(cbind(x.meanz.quantile[,1],x.meanz.mean,	x.meanz.quantile[,5], x.ssb.quantile[,1],x.ssb.mean,	x.ssb.quantile[,5], x.rec.quantile[,1],x.rec.mean,	x.rec.quantile[,5] ),3)
colnames(results)<-c("Z.low", "Z","Z.high", "SSB.low", "SSB", "SSB.high", "Rec.low", "Rec","Rec.high")

write.csv(results, "model\\SURBARsummary.csv")
