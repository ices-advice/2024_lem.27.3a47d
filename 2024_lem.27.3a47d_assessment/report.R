...
##         output/advice.Rdata
## After:  report/InterCatch LandPercent_2022.tiff
##         report/InterCatch DiscProvided_2022.tiff
##         report/InterCatch DisWt_2022.tiff
##         report/InterCatch LandWt_2022.tiff
##         
...
savePlot(filename = "report/InterCatch LandWt_2022",type = "tiff")
dev.off()
...
plotStockOverview(WtData,plotType="LandPercent",byFleet=TRUE,byCountry=TRUE,bySampled=TRUE,bySeason=FALSE,
                  byArea=FALSE,countryColours=TRUE,set.mar=TRUE,markSampled=TRUE,individualTotals=TRUE)
savePlot(filename = "report/InterCatch LandPercent_2022",type = "tiff")
dev.off()
...
plotStockOverview(WtData,plotType="DisWt",byFleet=TRUE,byCountry=TRUE,bySampled=TRUE,bySeason=FALSE,
                  byArea=FALSE,countryColours=TRUE,set.mar=TRUE,markSampled=TRUE,individualTotals=TRUE)
savePlot(filename = "report/InterCatch DisWt_2022",type = "tiff")
dev.off()
...
plotStockOverview(WtData,plotType="DiscProvided",byFleet=TRUE,byCountry=TRUE,bySampled=TRUE,bySeason=FALSE,
                  byArea=FALSE,countryColours=TRUE,set.mar=TRUE,markSampled=TRUE,individualTotals=TRUE)
savePlot(filename = "report/InterCatch DiscProvided_2022",type = "tiff")
...
fitwt <- data.frame(array(dim = c(19, 9))) #change dim 1
rownames(fitwt) <- 2005:2023               #extend by one year
colnames(fitwt) <- 1:9
...
{	if(i<4)maxW<-0.3 else maxW<-0.8
  ...
  plot(x$year, x$wt, pch = 16, xlab = "", ylab = "Weight (kg)",
       type = "n", ylim = c(0, maxW), xlim = c(2004, 2024))   # extend by one year
  x.i <- x[x$age == i,]
